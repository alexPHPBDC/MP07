...
function pintarTweets($usuari, $tweets, $frase = "")
{
    ...
    if (empty($tweets)) {
        ...
        foreach ($tweets as $tweet) {
            ...
            pintarLikeButton($usuari, $tweet->id,$tweet->userId);
            echo "</div><hr>";
        }
    }
}

function pintarLikeButton($usuari, $tweetId,$usuariAuxId)
{
    ...
    echo  "<form action='#'>
            <input id='usuariAuxId' type='hidden' name='idUsuari' value=$usuari->id>";

        echo "<input type='hidden' name='idAux' value=$usuariAuxId>";
    
        echo $tweetId;
    if (isset($usuari->likes[$tweetId])) {
        
        echo "<button onclick='ajaxLike(event,$usuari->id)' name='dislike' value=$tweetId>$heartFull</button>";
    } else {
        echo "<button onclick='ajaxLike(event,$usuari->id)' name='like' value=$tweetId>$heartGrey</button>";
    }
...
function pintarOwnTweets($usuari)
{
    if (empty($usuari->tweets)) {
        echo "<p>Encara no tens cap tweet! Fes click <a href='formularis/formulari_tweet.php' style='color:#990033;'>aqui</a> per crear-ne un 😊";
    } else {
...
function pintarTot($selfId, $auxId){
    $usuariAux = getUser($auxId);
    $usuari = getUser($selfId);
...
function changeAndPrint($selfId, $auxId,$changeAction){
    $frase = "<p>Aquest usuari encara no té cap tweet, anima'l a crear-ne un 😊";
    likeTweet($selfId, $changeAction);
    $usuariAux = getUser($auxId);
    $usuari = getUser($selfId);
    $tweets = getMissatgesOrderByDate($usuariAux);
    pintarTweets($usuari, $tweets, $frase);
